...
/* Data memory barrier -- REQUIRED on ARM1176 (ARMv6) around mailbox access,
   or the framebuffer address/pitch read back can be stale -> garbled output.
   This is the exact barrier from the known-good Pi 1 driver. */
static inline void fb_dmb(void){
#if defined(__aarch64__)
    __asm__ volatile("dmb sy" ::: "memory");
#elif defined(__arm__) && defined(__ARM_ARCH) && (__ARM_ARCH >= 7)
    __asm__ volatile("dmb" ::: "memory");
#elif defined(__arm__)
    __asm__ volatile("mcr p15, 0, %0, c7, c10, 5" :: "r"(0) : "memory"); /* ARMv6 DMB */
#else
...
static int mbox_call(uint32_t ch)
{
    ...
    for(;;){
        ...
        if(MBOX_READ == addr){ fb_dmb(); return mbox[1] == 0x80000000u; }
    }
}
...
{
    int i=0, fb_i, pitch_i, wh_i;
    mbox[i++]=0;                 /* total size, patched below           */
    mbox[i++]=0;                 /* request                              */
    mbox[i++]=0x00048003; mbox[i++]=8; mbox[i++]=0; wh_i=i; mbox[i++]=w; mbox[i++]=h;  /* set phys wh */
    ...
    /* Use what the GPU ACTUALLY gave us, not what we asked for: some boards/
       firmware negotiate a different mode. Fall back to the request if a field
       comes back zero. A wrong pitch is what shears text into garbage. */
    g_w     = mbox[wh_i]   ? mbox[wh_i]   : w;
    g_h     = mbox[wh_i+1] ? mbox[wh_i+1] : h;
    g_pitch = mbox[pitch_i] ? mbox[pitch_i] : g_w*4;
    return g_fb != 0;
}
